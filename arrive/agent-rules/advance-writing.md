...
- `status` is exactly one of **`planned | in_progress | complete | cancelled`**. **Never use `done`** — `done` is implementation-plan *item* vocabulary, not advance status (`complete` is the advance equivalent).
